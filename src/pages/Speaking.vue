<template>
  <Layout class="w-full">
    ...
    <h2 class="text-3xl mx-10 sm:text-left md:text-left my-5 text-center font-bold" data-aos="slide-up" v-if="speaking.upcoming">Upcoming Talks</h2>
      <p class="text-center">Want me to speak at your event? Reach out to me via email or twitter.</p>
    ...
    <div class="past sm:mx-4 md:mx-4  mx-64">
      ...
      <g-link :to="interviews.link">
        <div class="card rounded  px-8 py-6">
          ...
          <h2 class="text-2xl my-3 font-semibold">{{interviews.title}}</h2>
          <span class="font-bold">{{interviews.event}}</span>
        </div>
      </g-link>
      </div>
    </div>
